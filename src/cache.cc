...
/*  你不需要实现此功能，但需要将预取请求发送到下一级内存层次结构时调用它。
    默认情况下，预取器在缓存级别 N 生成的预取请求首先查找第 N 级缓存。
    未命中时，预取请求查找下一个缓存级别的缓存（N+1、N+2 等），如果未命中 LLC，则最终进入主存。
    一旦内存提供了相应的数据，数据将填充从 LLC 到第 N 级的所有缓存级别。  

    假设您的预取器在调用l*_prefetcher_operate方法期间“神奇地”想出了一个要预取的地址，它仍然需要发送预取请求。下面的方法可以完成这项工作。

    ip: 必须与l*_prefetcher_operate方法中的ip参数值相同
    base_addr: 必须与l*_prefetcher_operate方法中的参数addr相同。这基本上用于验证预取是否落在页面边界之外（如果是，请求被丢弃）
    pf_addr: 向其发出预取请求的缓存对齐地址。Bits[0:5]必须为0（因为块大小为64B）
    pf_fill_level: 填充预取地址的高速缓存级别。必须设置为FILL_L1 / FILL_L2 / FILL_LLC
    prefetch_metadata: 需要传递到较低缓存级别的预取器的任何64位信息。默认为 0
    return: 1. 如果成功发送预取请求: 返回1
            2. 如果预取请求由于前面提到的任何原因而被丢弃：返回0
*/
